...
const salesforceIdCandidate = /(?<![A-Za-z\d])(?:[A-Za-z\d]{15}|[A-Za-z\d]{18})(?![A-Za-z\d])/gu;
...
const PUBLIC_EVIDENCE_MAX_BYTES = 128 * 1024;
...
for (const path of await collect(liveRoot)) {
  ...
  if (Buffer.byteLength(contents) > PUBLIC_EVIDENCE_MAX_BYTES) {
    failures.push(`${relativePath}: oversized public live fixture`);
  }
  for (const [label, pattern] of patterns) {
    if (pattern.test(contents)) failures.push(`${relativePath}: ${label}`);
  }
  for (const [candidate] of contents.matchAll(salesforceIdCandidate)) {
    if (isSalesforce15(candidate) || isSalesforce18(candidate)) {
      failures.push(`${relativePath}: Salesforce record ID`);
      break;
    }
  }
  ...
  const inspectSecretKeys = (value, location = '', key = '') => {
    if (Array.isArray(value)) {
      if (value.length > PUBLIC_EVIDENCE_ARRAY_LIMIT) failures.push(`${relativePath}: oversized evidence array`);
      value.forEach((entry, index) => inspectSecretKeys(entry, `${location}[${index}]`, key));
      return;
    }
    if (typeof value === 'string') {
      try {
        const nested = JSON.parse(value);
        if (nested && typeof nested === 'object') inspectSecretKeys(nested, `${location}<json>`, key);
      } catch {}
      if (publicEvidenceRedactedKeys.test(key) && value !== '[REDACTED]') {
        failures.push(`${relativePath}: unredacted public evidence field ${location}`);
      }
      return;
    }
    ...
    for (const [key, entry] of Object.entries(value)) {
      if (secretKeys.test(key) && entry !== '[REDACTED]') {
        failures.push(`${relativePath}: secret-valued key ${location}.${key}`);
      } else {
        inspectSecretKeys(entry, `${location}.${key}`, key);
      }
    }
  };
