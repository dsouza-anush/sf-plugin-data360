...
export const assertPackageContents = async (tarball) => {
  ...
  if (invalidEntry) throw new Error(`Package contains unsafe archive entry: ${invalidEntry.path}.`);
  const entriesByPath = new Map(entries.map((entry) => [entry.path, entry]));
  if (entriesByPath.size !== entries.length) throw new Error('Package contains duplicate archive paths.');
  ...
  if (manifest.private === true) throw new Error('Package is marked private.');
  if (manifest.repository?.url !== 'git+https://github.com/dsouza-anush/sf-plugin-data360.git') {
    throw new Error('Package repository URL does not match the public GitHub repository.');
  }
  if (
    manifest.publishConfig?.access !== 'public' ||
    manifest.publishConfig?.registry !== 'https://registry.npmjs.org/'
  ) {
    throw new Error('Package publishConfig must target the public npm registry with public access.');
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.size > 8 * 1024 * 1024) throw new Error(`Package path ${entry.path} exceeds the 8 MiB content limit.`);
