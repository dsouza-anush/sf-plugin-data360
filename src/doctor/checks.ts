...
export const runDoctorChecks = async (dependencies: DoctorDependencies): Promise<DoctorCheck[]> => {
  ...
  try {
    ...
    checks.push(
      failure('Org authentication', error, 'Run sf org login web --alias <alias> --instance-url <my-domain-url>.')
    );
  }
    ...
    checks.push(
      {
        ...
        status: 'fail',
        detail: 'Direct checks were not configured; Connect API commands remain available.',
      ...
      },
      {
        ...
        action: 'Use Connect API commands or configure Direct API access.',
      },
      {
