...
export const RAW_RESPONSE_MAX_BYTES = 64 * 1024 * 1024;
...
export const readResponseBytes = async (
  response: Response,
  maximumBytes = RAW_RESPONSE_MAX_BYTES
): Promise<Uint8Array> => {
  const declaredLength = Number(response.headers.get('content-length'));
  if (Number.isFinite(declaredLength) && declaredLength > maximumBytes) {
    await response.body?.cancel();
    throw new Error(runtimeMessages.getMessage('error.RUNTIME_9.9'));
  }
  ...
  try {
    while (!done) {
      ...
      if (bytes > maximumBytes) throw new Error(runtimeMessages.getMessage('error.RUNTIME_9.9'));
      chunks.push(value);
    }
  } catch (error) {
...
export const writeResponseBodyFile = async (path: string, response: Response): Promise<void> => {
