...
describe('P1 contracts', () => {
  ...
  it('wires the seven release-verification tiers into scripts and non-live CI', async () => {
    ...
    expect(workflow).to.include('npm install --global @salesforce/cli@2.145.6');
  ...
  it('enforces clean-checkout build and explicit repository hook gates', async () => {
    ...
    expect(packageContents).to.include('unsafe archive entry');
    expect(packageContents).to.include('duplicate archive paths');
    expect(packageContents).to.include('exceeds the 8 MiB content limit');
    const publishWorkflow = await readFile(resolve(root, '.github', 'workflows', 'publish.yml'), 'utf8');
    expect(publishWorkflow).to.include('needs: validate');
    expect(publishWorkflow).to.include('npm publish ./sf-plugin-data360.tgz --access public');
    expect(publishWorkflow.indexOf('id-token: write')).to.be.greaterThan(publishWorkflow.indexOf('needs: validate'));
    expect(publishWorkflow).not.to.include('release:tag:check "${{ github.event.release.tag_name }}"');
    expect(publishWorkflow).to.include('git merge-base --is-ancestor HEAD refs/remotes/origin/main');
    const eca = await readFile(resolve(root, 'src', 'commands', 'data360', 'setup', 'eca.ts'), 'utf8');
    expect(eca).not.to.include('oauth.pstmn.io');
    const workflow = await readFile(resolve(root, '.github', 'workflows', 'test.yml'), 'utf8');
