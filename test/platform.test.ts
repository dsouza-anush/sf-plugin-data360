...
describe('P1 platform commands', () => {
  ...
  it('preserves raw bytes, includes response metadata, refreshes auth, and writes atomically', async () => {
    ...
    const streamed = join(await mkdtemp(join(tmpdir(), 'data360-response-stream-')), 'response.bin');
    await writeResponseBodyFile(streamed, new Response(new Uint8Array([7, 8, 9])));
    expect(await readFile(streamed)).to.deep.equal(Buffer.from([7, 8, 9]));
    expect(await readResponseBytes(new Response(new Uint8Array([1, 2, 3])), 3)).to.deep.equal(
      new Uint8Array([1, 2, 3])
    );
    await assertRejects(readResponseBytes(new Response(new Uint8Array([1, 2, 3])), 2), 'Use --stream-to-file');
    expect(RAW_RESPONSE_MAX_BYTES).to.equal(64 * 1024 * 1024);
  });
