...
describe('release secret scan', () => {
  ...
  it('detects Salesforce and Data 360 credential shapes without returning their values', async () => {
    ...
    const recordId = `${'001'}000000000001AAA`;
  ...
  it('allows GitHub Actions OIDC permission declarations but not token values', async () => {
    const { findSensitiveValues } = await loadScanner();
    expect(findSensitiveValues("id-token: write\nid_token: 'read'\nid-token: none\n")).to.deep.equal([]);
    expect(findSensitiveValues('id-token: opaque-production-token\n').map(({ label }) => label)).to.include(
      'credential-valued field'
    );
  });
