...
describe('P4 transform family', () => {
  ...
  it('bounds transform status refresh and returns the structured timeout recovery contract', async () => {
    const org = new MockTestOrgData('transform-timeout');
    await commandTest.context.stubAuths(org);
    let observedTimeout: number | undefined;
    commandTest.context.fakeConnectionRequest = async (request, options): Promise<never> => {
      const value = request as { url?: string };
      if (value.url?.endsWith('/data-transforms')) {
        return { dataTransforms: [{ name: 'OrdersTransform' }] } as never;
      }
      observedTimeout = (options as { timeout?: number } | undefined)?.timeout;
      throw Object.assign(new Error('Request timed out'), { name: 'AbortError' });
    };

    let timeoutError: (Error & { data?: { outcomeUnknown?: boolean } }) | undefined;
    try {
      await TransformReport.run(['-o', org.username, '-n', 'OrdersTransform', '--json']);
    } catch (error) {
      timeoutError = error as Error & { data?: { outcomeUnknown?: boolean } };
    }

    expect(observedTimeout).to.equal(30_000);
    expect(timeoutError).to.have.property('name', 'D360_JOB_TIMEOUT');
    expect(timeoutError?.data).to.include({ outcomeUnknown: true });
  });
});
