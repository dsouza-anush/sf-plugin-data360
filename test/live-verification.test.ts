...
describe('live verification orchestrator', () => {
  ...
  it('chooses the known mapped DMO and DLO pair from live list evidence', async () => {
    ...
    const dependencies = new Map<string, unknown>([
      [
        'data360 dmo list',
        JSON.parse(await readFile(resolve(root, 'test', 'fixtures', 'live', 'dmo-list.json'), 'utf8')),
      ],
      [
        'data360 dlo list',
        JSON.parse(await readFile(resolve(root, 'test', 'fixtures', 'live', 'dlo-list.json'), 'utf8')),
      ],
    ]);
