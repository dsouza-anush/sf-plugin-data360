...
Doctor checks org authentication, API-version alignment, Data 360 provisioning, data spaces, Direct API token exchange, a Direct API ping, required scopes, and the configured data space. It exits nonzero when any check fails. If only the Direct checks fail, Connect REST API commands can still be available, but you should not use ingestion, Direct token commands, or `api request --direct` until those failures are resolved.
