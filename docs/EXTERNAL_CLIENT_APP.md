...
Most core-org commands use the Salesforce session created by `sf org login web`. Direct Query, Profile, and Ingestion APIs additionally require a Salesforce External Client App (ECA) authorized for Data 360. The plugin performs Salesforce's two-step token exchange automatically and never requires you to paste a Data 360 tenant token into a command.
...
| `cdp_query_api`   | Direct Query API                                                 |
| `cdp_profile_api` | Profile API                                                      |
| `cdp_ingest_api`  | Streaming and bulk Ingestion APIs                                |

`sf data360 doctor` checks the four `cdp_*` scopes above. An exchange response can omit its scope inventory; in that case doctor reports a warning and only claims the Direct metadata access it actually verified.
