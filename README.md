...
The npm package becomes available when the matching GitHub release is published. To develop or evaluate the unreleased source instead:
...
See [docs/INSTALLATION.md](docs/INSTALLATION.md) for trust prompts, CI installation, updates, uninstalling, and source development.
...
| Data Kit          | `data-kit`                                                                                   | List, inspect, create, update, delete, deploy, and undeploy data kits                                        |
...
Streaming and bulk ingestion, Direct token commands, and `api request --direct` need an External Client App authorized with the scopes required by the selected Direct API. The typed `query` and `profile` commands use Connect REST API with normal Salesforce CLI org authentication. [docs/EXTERNAL_CLIENT_APP.md](docs/EXTERNAL_CLIENT_APP.md) explains the two authentication paths, admin setup, CI usage, and troubleshooting; `sf data360 doctor` validates both Connect and Direct readiness.
